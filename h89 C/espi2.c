...
/* Created 7/27/89
/* Last Modified: 6/20/2023 - Modified for ESP
/*    2/6/2023 - Added Ymodem
/*    6/26/89 - add 0Send File Xmodem mode
...
ReadXmodem()
  ...
  ok = FALSE;
  FlushCon();
  printf("\n\nFilename.Ext to get from ESP32 ? ");
  len = getline(filename, 16);
  for(k = 0; k < len; k++)
     filename[k] = toupper(filename[k]) ;
  printf("len: %d, Strlen: %d File: %s\n",len, strlen(filename), filename);
  ...
  if(ok)
      {
      printf("Sending File Request Command for %s\n", filename);
      StartCmd();
      Delay(10);
      Send(0x31);    /* Request File */
      for( j = 0; j < len; j++)
          Send(filename[j]);
      Send(0);
      ReceiveIt( fp );
...
SendFX( fp, filename )
  ...
  /* Send ESP32 Command 0x30 */
  StartCmd();
  Delay(5);
  Send( 0x30 );
  j = 0;
  while( j < 20 && filename[j] !=0)
      {
      printf("%d ",filename[j]);
      Send( filename[j++] );
      }
