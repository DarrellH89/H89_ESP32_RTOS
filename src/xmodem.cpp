...
//************** get H89 File ******************
// loads file on SD Card
// H89 provides filename
// file transmitted by xmodem
...
bool getH89File(String fname)
{
  ...
  // timeOutCounter = 10; // timeout counter set to 10 sec, Error at 8 sec. If reaches 0m, the ESP32 reboots
  // timeOutStart = millis();

  // while (currentStatus != ESP_BUSY)
  // { // wait until H89 reads data
  //   errors++;
  ...
  snum = 0;
  errors = 0;
  transize = STX_Size;      // data packet size
  ...
  { // get data loop
    ...
    first = 0;
    retryLast = retry = 0;
    while (!((first == SOH) || (first == EOT) || (first == STX)))
    {
      if (!getData(first))
        retry++;
      // delay(10)  ;
      if (DEBUG && retry % 30000 == 0 && k != dataInPtr && retry > retryLast)
      {
        ...
        // printDataBufPtr();
        Serial.printf("\n-------------- SOH/STX/EOT Retry: %d, Last: %d, Ptr: %d \n", retry, dataInLast, dataInPtr);
      }
      ...
      { /*<-- becomes this #*/
        ...
        {          
          ...
          {                      // get data packet
            ...
            // timeOutStart = millis();
            retry = 0;
            while (getDataTime(buffer[buffPtr], 50) == 0 )
              {
                retry++;
                if( retry > 5)
                {
          ...
          if ((getDataTime(chk1, TIMEOUT) == 0) || (getDataTime(chk2, TIMEOUT) == 0))
          { /* hi CRC byte */ /* lo CRC byte */
    ...
    {
      if (ok)
      { /* got a good sector and finished all processing so - */
      ...
      { /* some error occurred! */
        errors++;
        Serial.printf("Looking for SOH/STX or EOT, got: %d\n", first);
...
bool sendH89File(String fname)
{
  ...
  transize = STX_Size; /*  block transmission size */
  errmax = errormax;
  snum = 1;
  errors = 0;
  flagEOT = 0;
  ...
  do              // Wait for initial C to start
  {               /* check for initial 'C'' to start transfer */
    if (getDataTime(first, TIMEOUT) == 0)
    {
      errors++;
      if (DEBUG && errors % 5 == 0)
  ...
  if ((first == byte('C'))) /* okay to start transfer */
  {
    ...
    do // ************** Transfer loop ***************
    {
      ...
      timeOutCounter = HOLD; // stop timeout counter
      // Send data block
      crc = 0;
      ...
      if (first == ACK)
      {
        ...
        if( nbytes - buffPtr < STX_Size)  
        {
          if( transize > SOH_Size)
            Serial.printf("\nSwitched block size to 128\n");
          transize = SOH_Size;
        }
      }
